...
from nemo_gym.startup_keepalive import StartupKeepalive
...
class RunHelper:  # pragma: no cover
    ...
    _startup_keepalive: Optional[StartupKeepalive]
...
{proc_err}"""
    ...
    def _get_server_statuses_quiet(self) -> List[Tuple[str, ServerStatus]]:
        statuses = []
    ...
    def _ensure_startup_keepalive(self) -> None:
        if self._startup_keepalive is None:
            self._startup_keepalive = StartupKeepalive(
                get_statuses=self._get_server_statuses_quiet,
            )
            self._startup_keepalive.start()

    def stop_startup_keepalive(self) -> None:
        if self._startup_keepalive is not None:
            self._startup_keepalive.stop()
            self._startup_keepalive = None
    ...
    def wait_for_spinup(self) -> None:
        ...
        while True:
            ...
            self._ensure_startup_keepalive()
