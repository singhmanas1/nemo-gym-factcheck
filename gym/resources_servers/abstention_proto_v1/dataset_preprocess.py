...
"""
Preprocess HotPotQA into JSONL for the abstention_proto_v1 environment.
...
abstention_proto_v1 resources server.  Unlike the fact_hallucination_detection
preprocessor, this version:

  - Includes the ground-truth ``answer`` field in every record.
...
SYSTEM_PROMPT = (
    "Answer the following question. Put your final answer in \\boxed{your answer} format.\n"
    "If you are unsure about the answer, output \\boxed{[IDK]} instead of guessing.\n"
...
def build_record(row: dict, idx: int) -> dict:
    """Convert a single HotPotQA row into an abstention_proto_v1 record."""
    ...
    return {
        ...
        "agent_ref": {
            ...
            "name": "abstention_proto_v1_simple_agent",
        },
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess HotPotQA for abstention_proto_v1")
    parser.add_argument(
        "--download",
        action="store_true",
    ...
    parser.add_argument(
        "--raw-data-dir",
        type=Path,
        default=Path("/lustre/fsw/portfolios/llmservice/users/mfathi/data/hotpotqa"),
