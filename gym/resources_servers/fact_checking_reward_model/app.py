...
class FactCheckingRewardModelResourcesServerConfig(BaseResourcesServerConfig):
    kiwix_zim_dir: Optional[str] = None
    kiwix_port: int = 8080
    kiwix_serve_path: str
    kiwix_max_chars: int = 8000
    kiwix_top_k: int = 3
    kiwix_search_candidates: int = 100
    ...
    factuality_weight: float = Field(
        default=1.0,
        description="Multiplicative weight applied to the factuality component of the reward (base_reward = classification_accuracy * (1 + f1_score)).",
    )
    quality_weight: float = Field(
        default=1.0,
        description="Multiplicative weight applied to the quality component of the reward (quality_reward = -|ground_truth_quality - predicted_quality|).",
    )
...
class FactCheckingRewardModelResourcesServer(SimpleResourcesServer):
    ...
    def _extract_quality_score(output: str) -> Optional[float]:
        """Extract quality score from [Beginning of Quality Score]...[End of Quality Score] markers."""
        m = re.search(
    ...
    def _kiwix_search(self, query: str) -> list[dict]:
        """Search Kiwix with many candidates, re-rank by title similarity, return top kiwix_top_k."""
        n = max(self.config.kiwix_search_candidates, self.config.kiwix_top_k)
        resp = urllib.request.urlopen(
            f"{self._kiwix_url()}/search?pattern={urllib.request.quote(query)}&pageLength={n}",
            timeout=10,
        )
        ...
        for m in re.finditer(
            ...
            results.append({
                ...
                "title_score": difflib.SequenceMatcher(
                    None, query.lower(), title.lower()
                ).ratio(),
            })
        results.sort(key=lambda r: r["title_score"], reverse=True)
        return results[: self.config.kiwix_top_k]
    ...
    def _kiwix_direct_lookup(self, query: str) -> Optional[dict]:
        """Try a direct /A/<title> lookup for exact entity matches. Returns article dict or None."""
        # Kiwix serves articles at /A/<Title_With_Underscores>
        title = query.strip().replace(" ", "_")
        path = f"/A/{urllib.request.quote(title)}"
        try:
            urllib.request.urlopen(f"{self._kiwix_url()}{path}", timeout=5)
            return {"title": query.strip(), "url": path}
        except Exception:
            return None
    ...
    async def search_wiki(self, body: SearchWikiRequest) -> SearchWikiResponse:
        ...
        try:
            # Try exact title lookup first — most reliable for entity names
            parts = []
            seen_urls = set()
            direct = self._kiwix_direct_lookup(body.query)
            if direct:
                article = self._kiwix_fetch_article(direct["url"])
                parts.append(f"=== Wikipedia: {direct['title']} ===\n\n{article}")
                seen_urls.add(direct["url"])

            # Fill remaining slots from re-ranked search results
            ...
            for r in results:
                if len(parts) >= self.config.kiwix_top_k:
                    break
...
if __name__ == "__main__":
    FactCheckingRewardModelResourcesServer.run_webserver()
