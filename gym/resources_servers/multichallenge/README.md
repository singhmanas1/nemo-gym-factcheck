...
1. **Copy raw data files**:
   ```bash
   cp -r /path/to/multichallenge/advanced data/advanced
   cp -r /path/to/multichallenge/vanilla data/vanilla
   ```

2. **Preprocess to JSONL format**:
   ```bash
   python dataset_preprocess.py
   ```
   
   This converts:
   - `data/advanced/*.json` → `data/advanced.jsonl` (113 tasks)
   - `data/vanilla/*.json` → `data/vanilla.jsonl` (111 tasks)
